...
class ResetPass extends Controller
{
    ...
    public function reset(Request $req)
    {
        ...
        if (Cache::has('reset-' . $email)) {
            return back()->with('error', 'We have already sent you an email with a validation link. Try again later for another reset request.');
        } else {
            ...
            try {
                \Mail::send(array(), array(), function ($message) use ($html, $email) {
                    $message->to($email)->subject('Password reset')->setBody($html, 'text/html');
                });
                Cache::put('reset-' . $email, 1, 3600);
    ...
    public function apiReset(Request $req)
    {
        try {
            ...
            if (Cache::has('reset-' . $email)) {
                ...
                return ['status'=> 0, 'message' => 'We have already sent you an email with a validation link. Try again later for another reset request.'];
    ...
    public function makeReset($type, $key = null)
    {
        if ($key && Cache::has($key)) {
            ...
            Cache::forget($key);
            if ($type == 'm') {
                return ['status'=> 1, 'message' => 'Password reset was successful, now you can login with your new password.'];
            }
            return redirect()->route('forget')->with('success', 'Password reset was successful, now you can login with your new password.');
        } else {
            if ($type == 'm') {
                return ['status'=> 0, 'message' => 'Either reset validation link expired or invalid reset link!'];
            }
