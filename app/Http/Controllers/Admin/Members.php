...
class Members extends Controller
{
    ...
    {
        $btn = '<a href="'.route('members').'?online=1" class="btn btn-outline-primary small-btn">المستخدمون النشطون أولاً</a>';
        if ($req->has('online') && $req->get('online') == 1) {
            $users = DB::table('users')->orderBy('updated_at', 'desc')->paginate(24);
            $btn = '<a href="'.route('members').'" class="btn btn-outline-primary active small-btn">المستخدمون النشطون أولاً</a>';
        } elseif ($req->has('cc')) {
        ...
        $htmlCode = '';
        foreach ($users as $u) {
            $readableTime = "اخر ظهور".Carbon::parse($u->updated_at)->diffForHumans();
            ...
            if ($u->avatar == null) {
                ...
                $online = '<span class="avatar rounded avatar-md flag-country-'. strtolower($u->country).'">'.$online.'</span>';
            } else {
            ...
            $htmlCode .= '<div class="col-lg-3 col-md-4 col-sm-6 col-12">
                            ...
                            <a class="card card-link" href="'.route('userinfo', ['userid' => $u->userid]).'">
                                ...
                                <div class="card-body align-items-center d-flex">
                                    ...
                                    <div class="float-left mr-3">'. $online .' </div>
                                    
                                   
                                    <div class="lh-sm"> 
                                        <div class="strong text-truncate m-2">'.$u->name.'</div>
                                        <div class="text-muted text-truncate m-1">
                                            <h5 class="my-0 line-height-small m-1">'.$u->email.'</h5>
                                            <h5 class="my-0 line-height-small m-1">'.$readableTime.'</h5>
                                        </div>
                                    </div>
                                </div>
                            </a>
        ...
        $data = ['htmlcode' => $htmlCode, 'users' => $users, 'btn' => $btn, 'title' => 'دليل المستخدمين'];
    ...
    public function userInfo(Request $req)
    {
        try {
            ...
            $data = [
				...
				'multi' => DB::table('users')->where('device_id', $userinfo->device_id)->where('id', '!=', $userinfo->id)->orderBy('id', 'desc')->get()
				
            ];
            return view('admin.member_info', compact('data'));
        } catch (\Exception $e) {
           // return back()->with('error', 'Unknown error occurred!');
             return ['code' => 404,'status' => 0, 'message' => 'Unexpected error occurred!'.$e->getMessage()];
        }
    }
    ...
    public function userinfoUpdate(Request $req)
    {
        ...
        if ($check) {
            if ($ref == null) {
                ...
                return back()->with('error', 'لا يمكن للمستخدم نفسه أن يكون المُحيل له!');
            }
            $chk = DB::table('hist_activities')->where('note', $ref)->where('network', 'referral')->first();
            if ($chk && $chk->userid == $check->userid) {
               return back()->with('error', 'رمز الإحالة هذا "'.$ref.'" لا يمكن استخدامه. لا يمكن للمستخدمين إحالة بعضهم البعض');
            }
            if ($ref == 'none' && $check->refby != 'none') {
                ...
                if ($rf) {
                    ...
                    return back()->with('error', 'معرف المرجع "'.$ref.'" غير معثور عليه!');
                }
            }
            $db->update([
                'name' => $req->post('name'),
            ...
            return back()->with('success', 'تم تحديث معلومات ملف تعريف المستخدم بنجاح.');
        } else {
            return back()->with('error', 'لم يتم العثور على المستخدم!');
        }
    }
    ...
    public function Reward(Request $req)
    {
        ...
        if ($p < 1) {
            return back()->with('error', 'يجب أن يكون المبلغ أكبر من الصفر!');
        }
        ...
        return back()->with('success', 'تمت مكافأة المستخدم بـ '.$p.' '.strtolower(env('CURRENCY_NAME')).'s');
    }
    ...
    public function Penalty(Request $req)
    {
        ...
        return back()->with('success', $p.' '.strtolower(env('CURRENCY_NAME'))."تم خصم s من رصيد المستخدم");
    }
}
