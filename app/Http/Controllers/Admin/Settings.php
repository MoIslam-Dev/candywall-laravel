...
class Settings extends Controller
{
    ...
    {
           $data = [
            'apf_reward' => Funcs::getmisc('apf_reward'),
            'dev_key' => Funcs::getmisc('dev_key')
        ];
    ...
    public function update(Request $req)
    {
        $this->validate($req, [
            ...
            'apf_event_reward' => 'required|integer|min:0'
        ]);
        ...
        Funcs::setEnv('LEADERBOARD_REWARD', $req->post('leaderboard_reward'));
         ...
         Funcs::setmisc('apf_reward', $req->post('apf_event_reward'));
        $dev_key = $req->post('dev_key');
        if($dev_key == null){
            DB::table('misc')->where('name', 'dev_key')->delete();
        } else {
            Funcs::setmisc('dev_key', $req->post('dev_key'));
        }
    ...
    public function updateLB(Request $req)
    {
        ...
        if ($c == 100) {
            ...
            return ['status' => 0, 'message' => 'جمع '.$d.' مساوي ل '.$c.'. يجب أن يكون 100.'];
        }
    }
    ...
    public function geoUpdate(Request $req)
    {
        ...
        if ($dl1 == null && $dl2 != null) {
            return back()->withInput()->with('error', 'املأ الحقل "أول مفتاح كائن JSON عميق" لملء الحقل الثاني!');
        }
        ...
        if ($status == 200) {
            try {
                ...
                if (Funcs::countryExist($cc)) {
                    ...
                    return back()->with('success', 'تم تحديث الرابط والتحقق منه بنجاح');
                } else {
                    return back()->withInput()->with('error', 'فشل التحقق من واجهة برمجة التطبيقات. تأكد من صحة بياناتك المدخلة!');
                }
            } catch (\Exception $e) {
                return back()->withInput()->with('error', 'لا يمكن تحليل بيانات json!');
            }
        } else {
            return back()->with('error', 'استجاب الموفر برمز الخطأ هذا: '.$status);
        }
    }
	...
	public function emailUpdate(Request $req)
    {
        $this->validate($req, [
            ...
            'username' => 'nullable|string|regex:/^\S*$/u',
            'password' => 'nullable|string|regex:/^\S*$/u',
            'encryption' => 'nullable|string|regex:/^\S*$/u',
            'from_address' => 'nullable|string|regex:/^\S*$/u'
        ]);
        ...
        return back()->with('success', 'تم تحديث إعداد SMTP');
    }
}
