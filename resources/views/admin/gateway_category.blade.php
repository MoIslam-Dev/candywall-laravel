...
<div class="page-header">
    <div class="row align-items-center">
        <div class="col-auto">
            <h2 class="page-title">اعداد بوابة الدفع</h2>
        </div>
    </div>
</div>
    ...
    <form class="col-xl-4 col-md-5 col-sm-12" action="{{route('gateway_category_add')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="card">
            <div class="card-header bg-gray-lt pt-3 pb-2">
                <h4 class="text-dark">اضف طريقة دفع</h4>
            </div>
            <div class="card-body pt-2 row">
                <div class="mb-3">
                    <label class="form-label">اسم طريقة الدفع</label>
                    <input type="text" class="form-control" name="name" placeholder="بطاقة هدايا أمازون" value="{{old('name')}}">
                </div>
                <div class="mb-3">
                    <div class="form-label">صورة الدفع</div>
                    <div class="form-file">
                        <input type="file" name="image" class="form-file-input modal-img-input" id="customFile">
                        <label class="form-file-label" for="customFile">
                            <span class="form-file-text modal-img-choose">اختر صورة...</span>
                            <span class="form-file-button">استعراض</span>
                        </label>
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label">تاكيد نص الحوار:</label>
                    <input type="text" class="form-control" name="input_desc" placeholder="أدخل عنوان بريدك الإلكتروني لتلقي بطاقة الهدايا" value="{{old('input_desc')}}">
                </div>
                <div class="mb-3">
                    <div class="form-label">مدخلات مربع الحوار:</div>
                    <select class="form-select" name="input_type">
                        <option value="1">نص</option>
                        <option value="2" selected>بريد الكتروني</option>
                        <option value="3">رقم</option>
                    </select>
                </div>
                <div>
                    <label class="form-label" name="country">الدول المسموح بها:</label>
                    <input type="text" class="form-control" name="country" placeholder="US,UK,GB" value="{{old('country')}}">
                </div>
            </div>
            <div class="card-footer">
                <button type="submit" class="btn btn-success">اضافة هذه البطاقة</button>
            </div>
        </div>
    </form>
    <div class="col-lg-8 col-md-7 col-sm-12">
        <div class="alert alert-info">انقر على فئة لإدارة عناصرها.</div>
		<div class="alert alert-danger">لا تقم بإضافة أي عملة حقيقية في فئة الهدايا هذه.</div>
        <div class="row">
            @foreach($data as $c)
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <div>أنت على وشك إزالة هذه الفئة وجميع أسئلتها من قاعدة بياناتك.</div>
            </div>
...
<form method="post" action="{{route('gateway_category_edit')}}" enctype="multipart/form-data" class="modal modal-blur fade" id="cat-edit" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content card">
            <div class="card-body">
                <div class="mb-3">
                    <label class="form-label">اسم فئة البطاقة</label>
                    <input type="text" class="form-control" name="name" id="edit-name">
                </div>
                <div class="mb-3">
                    <div class="form-label">صورة البطاقة</div>
                    <div class="form-file">
                        <input type="file" name="image" class="form-file-input modal-img-input" id="customFile">
                        <label class="form-file-label" for="customFile">
                            <span class="form-file-text modal-img-choose">اختر صورة...</span>
                ...
                <div class="mb-3">
                    <div class="form-label">نوع مدخلات مربع الحوار:</div>
                    <select class="form-select" name="input_type">
                        <option id="edit-ty" selected></option>
                        <option value="1">نص</option>
                        <option value="2">بريد الكتروني</option>
                        <option value="3">ارقام</option>
                    </select>
                </div>
                <div>
                    <label class="form-label" name="country">فرض الدولة:</label>
                    <input type="text" class="form-control" name="country" id="edit-ct">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary mr-auto" data-dismiss="modal">الغاء</button>
                <button type="submit" class="btn btn-success">تحديث صنف البطاقة</button>
            </div>
        </div>
    </div>
</form>
