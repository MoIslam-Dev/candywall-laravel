...
Route::get('datadelete', 'HomeController@dataDelete')->name('ddel');
...
Route::get('login/reset/{type}/{dta?}', 'Auth\ResetPass@makeReset')->where('dta', '(.*)');
Route::get('register/confirm/{type}/{dta?}', 'Auth\RegisterController@confirmReg')->where('dta', '(.*)');

Route::get('app-ads.txt', 'HomeController@appAds')->name('app_ads');

Route::group(['middleware' => 'isAdmin'], function () {
    Route::get('index', 'Admin\Index@index')->name('index');
	Route::get('index/dms', 'Admin\Index@indexWrn')->name('index_wrn');
	...
	Route::post('app/ads', 'Admin\Maintain@appAdsUpdate')->name('app_ads_update');
	...
	Route::post('game/scratch/limit', 'Admin\GameScratch@setLimit')->name('game_scratch_limit');
	...
	//chat
    Route::get('chat', 'Admin\Chat@view')->name('chat');
    Route::post('chatquick', 'Admin\Chat@quick')->name('chat_quick');
    Route::post('chat/send', 'Admin\Chat@send')->name('chat_send');
    Route::post('chat/del', 'Admin\Chat@del')->name('chat_del');
    Route::get('chat/delall', 'Admin\Chat@delAll')->name('chat_del_all');
    Route::post('chat/update', 'Admin\Chat@update')->name('chat_update');
	Route::get('/chat/media/{name}', 'Admin\Chat@adminAttachment');
});